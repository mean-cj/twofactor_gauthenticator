if (window.rcmail) {
  rcmail.addEventListener("init", function (evt) {
    ...
    function setup2FAfields() {
      ...
      $("input.btn-save").attr("disabled", "disabled").attr("title", rcmail.gettext("check_code_to_activate", "twofactor_gauthenticator"));
    ...
    $("#2FA_check_code").click(function () {
      ...
      $.post(url, function (data) {
        alert(data);
        if (data == rcmail.gettext("code_ok", "twofactor_gauthenticator")) $(".btn-save").removeAttr("disabled");
      });
    });

    // Define Variables
    var tabtwofactorgauthenticator = $("<li>").attr("id", "settingstabplugintwofactor_gauthenticator").addClass("listitem twofactor_gauthenticator");
    var button = $("<a>")
