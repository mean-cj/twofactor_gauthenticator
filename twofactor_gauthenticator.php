...
class twofactor_gauthenticator extends rcube_plugin
{
    ...
    private function __get2FAconfig()
    {
        ...
        if (!is_array($data) && $rcmail->config->get('twofactor_pref_encrypt'))
        {
            ...
            if ($cdata == null)
            {
                rcube::write_log('twofactor_gauthenticator', "WARN: [" . $rcmail->user->data['username'] . "] Broken 2FA!, clearing... - rip:" . $_SERVER['HTTP_X_FORWARDED_FOR'] . " lip:" . $_SERVER['REMOTE_ADDR']);
                $cdata = array();
            }
            $data = (array)$cdata;
        }
    ...
    private function __set2FAconfig($data)
    {
        ...
        rcube::write_log('twofactor_gauthenticator', "WARN: [" . $rcmail->user->data['username'] . "] 2FA may have changed to " . $data['activate'] . " ! - rip:" . $_SERVER['HTTP_X_FORWARDED_FOR'] . " lip:" . $_SERVER['REMOTE_ADDR']);
    ...
    {
        $rcmail = rcmail::get_instance();
        rcube::write_log('twofactor_gauthenticator', "ERROR: [" . $rcmail->user->data['username'] . "] 2FA fail - rip:" . $_SERVER['HTTP_X_FORWARDED_FOR'] . " lip:" . $_SERVER['REMOTE_ADDR']);
    }
}
